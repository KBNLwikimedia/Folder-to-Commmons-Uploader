@echo off
REM ------------------------------------------------------------
REM Folder-to-Commons-Uploader (Windows launcher v2 - headless monitor)
REM - Creates/activates .venv
REM - Upgrades pip, installs requirements
REM - Ensures data store
REM - Starts monitor.py via pythonw.exe (no console, background)
REM - Starts app.py (blocks in this window)
REM ------------------------------------------------------------
...
REM --- Verify pythonw.exe exists (should be created with venv) ---
if not exist ".venv\Scripts\pythonw.exe" (
  echo [ERROR] Could not find pythonw.exe in .venv\Scripts
  echo Try recreating the venv: rmdir /s /q .venv  ^&  %PYLAUNCHER% -m venv .venv
  exit /b 1
)
...
REM --- Start monitor.py HEADLESS (no console window) ---
echo Starting monitor.py (background, no console)...
start "" /d "%CD%" "%CD%\.venv\Scripts\pythonw.exe" "%CD%\monitor.py"
